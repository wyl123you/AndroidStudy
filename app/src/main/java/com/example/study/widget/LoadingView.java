...
public class LoadingView extends View {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        // 半个⽅块的宽度（dp）
        half_BlockWidth = typedArray.getDimension(R.styleable.LoadingView_half_BlockWidth, 30);
        // ⽅块间隔宽度（dp）
        blockInterval = typedArray.getDimension(R.styleable.LoadingView_blockInterval, 10);
        // 移动⽅块的圆⻆半径
        moveBlock_Angle = typedArray.getFloat(R.styleable.LoadingView_moveBlock_Angle, 10);
        // 固定⽅块的圆⻆半径
        fixBlock_Angle =
                typedArray.getFloat(R.styleable.LoadingView_fixBlock_Angle, 30);
        // 通过设置两个⽅块的圆⻆半径使得⼆者不同可以得到更好的动画效果哦
        // ⽅块颜⾊（使⽤⼗六进制代码，如#333、#8e8e8e）
        int defaultColor = ResourcesCompat.getColor(getResources(), R.color.colorAccent, null);
        //默认颜⾊
        ...
        // 由于移动⽅块只能是外部⽅块，所以这⾥需要判断⽅块是否属于外部⽅块
        if (isInsideTheRect(initPosition, lineNumber)) {
            initPosition = 0;
        }
        // 动画⽅向是否 = 顺时针旋转
        isClock_Wise = typedArray.getBoolean(R.styleable.LoadingView_isClock_Wise, true);
        // 移动⽅块的移动速度
        // 注：不建议使⽤者将速度调得过快
        // 因为会导致ValueAnimator动画对象频繁重复的创建，存在内存抖动
        moveSpeed = typedArray.getInteger(R.styleable.LoadingView_moveSpeed, 250);
        // 设置移动⽅块动画的插值器
        int move_InterpolatorResId = typedArray.getResourceId(R.styleable.LoadingView_move_Interpolator, android.R.anim.linear_interpolator);
        move_Interpolator = AnimationUtils.loadInterpolator(context, move_InterpolatorResId);
        // 当⽅块移动后，需要实时更新的空⽩⽅块的位置
     ...
     * 关注1
     * 初始化⽅块对象、之间的关系
     * 参数说明：initPosition = 移动⽅块的初始位置
     */
    private void initBlocks(int initPosition) {
        // 1. 创建总⽅块的数量（固定⽅块） = lineNumber * lineNumber
        // lineNumber = ⽅块的⾏数
        // fixedBlock = 固定⽅块 类 ->>关注2
        ...
        // 4. 关联外部⽅块的位置
        // 因为外部的⽅块序号 ≠ 0、1、2…排列，通过 next变量（指定其下⼀个），
        // ⼀个接⼀个连接 外部⽅块 成圈
        // ->>关注4
    ...
    private static class FixedBlock {
        ...
        // 指向下⼀个需要移动的位置
        FixedBlock next;
        // 外部的⽅块序号 ≠ 0、1、2…排列，通过 next变量（指定其下⼀个），⼀个
        // 接⼀个连接 外部⽅块 成圈
    }
    // 请回到原处
    ...
    /**
     * 关注4：将外部⽅块的位置关联起来
     * 算法思想： 按照第1⾏、最后1⾏、第1列 & 最后1列的顺序，分别让每个外部
     * ⽅块的next属性 == 下⼀个外部⽅块的位置，最终对整个外部⽅块的位置进⾏关联
     * 注：需要考虑移动⽅向变量isClockwise（ 顺 Or 逆时针）
     */
    private void relate_OuterBlock(FixedBlock[] fixedBlocks, boolean isClockwise) {
        ...
        for (int i = 0; i < lineCount; i++) {
            // 位于最左边
            if (i % lineCount == 0) {
                fixedBlocks[i].next = isClockwise ? fixedBlocks[i + lineCount] :
                        fixedBlocks[i + 1];
                // 位于最右边
            } else if ((i + 1) % lineCount == 0) {
                fixedBlocks[i].next = isClockwise ? fixedBlocks[i - 1] : fixedBlocks[i +
                        lineCount];
                // 中间
            } else {
        ...
        for (int i = 1 * lineCount; i <= (lineCount - 1) * lineCount; i += lineCount) {
            ...
            fixedBlocks[i].next = isClockwise ? fixedBlocks[i + lineCount] : fixedBlocks[i
                    - lineCount];
        }
        // 情况4：关联最后1列
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        // 调⽤时刻：onCreate之后onDraw之前调⽤；view的⼤⼩发⽣改变就会调⽤该⽅法
        // 使⽤场景：⽤于屏幕的⼤⼩改变时，需要根据屏幕宽⾼来决定的其他变量可以在这⾥进⾏初始化操作
     ...
     * 关注1：设置 固定⽅块位置
     */
    private void fixedBlockPosition(FixedBlock[] fixedBlocks, int cx, int cy, float
            dividerWidth, float halfSquareWidth) {
        // 1. 确定第1个⽅块的位置
        // 分为2种情况：⾏数 = 偶 / 奇数时
        // 主要是是数学知识，此处不作过多描述
        ...
        for (int i = 0; i < lineCount; i++) {//⾏
            for (int j = 0; j < lineCount; j++) {//列
                if (i == 0) {
                    ...
                    int currIndex = i * lineCount + j;
                    fixedBlocks[currIndex].rectF.set(fixedBlocks[currIndex -
                            lineCount].rectF);
    ...
    private void MoveBlockPosition(FixedBlock[] fixedBlocks,
        ...
        // 移动⽅块位置 = 设置初始的空出位置 的下⼀个位置（next）
        // 下⼀个位置 通过 连接的外部⽅块位置确定
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mfixedBlocks.length; i++) {
            ...
            if (mfixedBlocks[i].isShow) {
                // 传⼊⽅块位置参数、圆⻆ & 画笔属性
                canvas.drawRoundRect(mfixedBlocks[i].rectF, fixBlock_Angle,
                        fixBlock_Angle, mPaint);
            }
        }
        // 2. 绘制移动的⽅块（）
        if (mMoveBlock.isShow) {
            canvas.rotate(isClock_Wise ? mRotateDegree : -mRotateDegree,
                    mMoveBlock.cx, mMoveBlock.cy);
            canvas.drawRoundRect(mMoveBlock.rectF, moveBlock_Angle,
                    moveBlock_Angle, mPaint);
        }
    }
    ...
    public void startMoving() {
        ...
        mAnimatorSet.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationStart(Animator animation) {
                // 每次动画开始前都需要更新移动⽅块的位置 ->>关注4
                updateMoveBlock();
                // 让移动⽅块的初始位置的下个位置也隐藏 = 两个隐藏的⽅块
    ...
    private ValueAnimator createTranslateValueAnimator(FixedBlock
        ...
        // 1. 设置移动速度
        ValueAnimator valueAnimator = new
                ValueAnimator().setDuration(moveSpeed);
        // 2. 设置移动⽅向
        // 情况分为：4种，分别是移动⽅块向左、右移动 和 上、下移动
        // 注：需考虑 旋转⽅向（isClock_Wise），即顺逆时针 ->>关注1.1
        if (isNextRollLeftOrRight(currEmptyfixedBlock, moveBlock)) {
            // 情况1：顺时针且在第⼀⾏ / 逆时针且在最后⼀⾏时，移动⽅块向右移动
     ...
     * 即上下 or 左右
     */
    private boolean isNextRollLeftOrRight(FixedBlock currEmptyfixedBlock,
                                          FixedBlock rollSquare) {
    ...
    /**
     * 关注2：实时更新移动⽅块的旋转中⼼
     * 因为⽅块在平移旋转过程中，旋转中⼼也会跟着改变，因此需要改变
     * MoveBlock的旋转中⼼（cx,cy）
     */
    private void setMoveBlockRotateCenter(MoveBlock moveBlock, boolean
            isClockwise) {
        // 情况1：以移动⽅块的左上⻆为旋转中⼼
        ...
        //以下判断与旋转⽅向有关：即顺 or 逆顺时针
        // 情况1：左边
        else if (moveBlock.index % lineNumber == 0) {
            moveBlock.cx = moveBlock.rectF.right;
            moveBlock.cy = isClockwise ? moveBlock.rectF.top :
                    moveBlock.rectF.bottom;
            // 情况2：上边
            ...
            moveBlock.cx = moveBlock.rectF.left;
            moveBlock.cy = isClockwise ? moveBlock.rectF.bottom :
                    moveBlock.rectF.top;
            // 情况4：下边
    ...
    private ValueAnimator createMoveValueAnimator() {
        // 通过属性动画进⾏设置
        ValueAnimator moveAnim = ValueAnimator.ofFloat(0,
                90).setDuration(moveSpeed);
